...
- `pages/admin/trip.html?id=<tripId>`: **relatório completo de um turno**. Cabeçalho, grid de indicadores, lista completa de paradas, despesas (com lightbox de recibo) e avarias do turno (`listDamagesByTrip`, com lightbox). Botão **"Exportar PDF"** = `window.print()` + `css/pages/report.css` (`@media print`: fundo branco, sem cromo de app, cards sem quebra, título "Relatório de Turno — placa — data"). Sem dependência de PDF — o iOS tem "imprimir em PDF" nativo (folha de impressão → pinch na prévia → Compartilhar → Salvar em Arquivos).
  - **iOS PWA**: no app instalado (modo standalone), `window.navigator.standalone === true`, o `window.print()` não faz nada **e** `window.open()` via script também costuma ser bloqueado (não dá pra confiar num "abre em nova aba" feito por JS). Por isso, nesse caso, os botões de Exportar PDF são trocados por **links `<a target="_blank">` de verdade** (`turnIntoPrintLink()` em `trip.js`) apontando pro próprio relatório com `?print=1` — toque real de link é a única forma que o iOS realmente respeita pra sair do modo standalone. A aba que abre (já fora do standalone) detecta `?print=1` e dispara o `window.print()` sozinha, depois de esperar as imagens carregarem (teto de 3s). No Safari normal e no Android o `window.print()` roda direto, sem desvio.
...
- **Strings de data pura (`"YYYY-MM-DD"`, ex.: `trip.date`, `maintenance.date`, `vehicle.nextRevisionDate`/`lastRevisionDate`) nunca via `new Date(str)` direto** — o construtor trata isso como meia-noite UTC e a exibição recua um dia em fusos negativos (Brasil, UTC-3). `toDate()`/`formatDate()` em `utils.js` já tratam isso (monta a data no fuso local) — sempre passar essas strings por ali, nunca `new Date(dateString)` cru.
